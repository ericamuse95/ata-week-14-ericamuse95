...
public class ReceiptDAO {
    ...
    public BigDecimal getSalesBetweenDates(ZonedDateTime fromDate, ZonedDateTime toDate) {
        //TODO scan for receipts between the two dates
        //TODO Cycle through all those returned receipts and gather the salesTotal into one result
            ...
            BigDecimal receiptResults = BigDecimal.valueOf(0);
            for(Receipt receipt : result){
                receiptResults = receiptResults.add(receipt.getSalesTotal());
            }
    ...
    public List<Receipt> getReceiptsPaginated(int limit, Receipt exclusiveStartKey) {
            ...
            ScanResultPage<Receipt> resultsPage = mapper.scanPage(Receipt.class, scanExpression);
